...
def main():
    parser = argparse.ArgumentParser(description="Конвертеры данных") #Создаётся объект ArgumentParser с текстом описания.
    sub = parser.add_subparsers(dest="cmd", required=True)#add_subparsers создаёт подкоманды (subcommands) — это позволяет командам вида script.py json2csv
    ...
    # --- json2csv ---
    p1 = sub.add_parser("json2csv", help="Конвертировать JSON → CSV")#add_parser добавляет новую подкоманду в парсер.
    p1.add_argument("--in", dest="input", required=True, help="Путь к входному JSON-файлу")#•	--in — флаг, который пользователь пишет в командной строке,dest="input" — имя, под которым значение будет храниться в args.
    ...
    args = parser.parse_args()#происходит разбор аргументов командной строки и создание объекта Python, где каждый аргумент доступен как атрибут.
...
if __name__ == "__main__":#который проверяет, запущен ли скрипт напрямую или импортирован как модуль.
    main()